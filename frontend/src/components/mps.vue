<template>
  <div id="main">
    <div id="popAdd" v-if="showAdd">
      ...
      <div id="body">
        <div style="flex:1;">
          <div>产品名称<br><input type="text" v-model="name"></div>
          <div>完工日期<br><input type="text" v-model="date" placeholder="格式:YY-MM-DD"></div>
        </div>
        <div style="flex:1;">
          <div>数量<br><input type="text" v-model="num"></div>
        </div>
      </div>
      <div id="footer">
...
export default{
  ...
  methods:{
		getdata(){
			this.axios.get('http://localhost:8000/api/mps/')
			.then(res=>{
    ...
    add_commit(){
      this.showAdd=0;
			this.axios({
        method:"post",
        url:"http://localhost:8000/api/add_mps/",
        data:{
          name:this.name,
          num:this.num,
          date:this.date,
    ...
    clear(){
      this.axios.get('http://localhost:8000/api/clear_mps/')
			.then(res=>{
				this.content=res.data
			});
      setTimeout(() => {
