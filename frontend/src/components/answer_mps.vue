<template>
  <div id="main">
    ...
    <div id="right">
      <div id="top">
        <ul>
          <li><h1>&ensp;<i class="fa fa-shield"/>&ensp;计算结果</h1></li>
          <li><router-link to="/mps"><button id="clear">返回</button></router-link></li>
        </ul>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="i in content">
              ...
              <td>{{ i.name }}</td>
              <td>{{ i.num }}</td>
              <td>{{ i.date_start.split("T")[0] }}</td>
              <td>{{ i.date_end.split("T")[0] }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default{
  ...
  methods:{
		getdata(){
			this.axios.get('http://localhost:8000/api/answer_mps/')
			.then(res=>{
